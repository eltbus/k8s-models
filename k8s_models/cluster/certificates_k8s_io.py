...
class CertificateSigningRequest(BaseModel):
    ...
    spec: CertificateSigningRequestSpec = Field(
        default=None,
        description=r""" spec contains the certificate request, and is immutable after creation. Only the request, signerName, expirationSeconds, and usages fields can be set on creation. Other fields are derived by Kubernetes and cannot be modified by users. """,
    )
    status: CertificateSigningRequestStatus = Field(
        default=None,
        description=r""" status contains information about whether the request is approved or denied, and the certificate issued by the signer, or the failure condition indicating signer failure. """,
    )
