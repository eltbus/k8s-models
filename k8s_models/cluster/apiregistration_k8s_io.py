...
class APIService(BaseModel):
    ...
    spec: APIServiceSpec = Field(
        default=None,
        description=r""" Spec contains information for locating and communicating with a server """,
    )
    status: APIServiceStatus = Field(
        default=None,
        description=r""" Status contains derived information about an API server """,
    )
