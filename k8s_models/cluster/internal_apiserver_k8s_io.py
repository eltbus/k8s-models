...
from k8s_models.cluster.apiserverinternal import StorageVersionSpec
from k8s_models.definitions.unknown import StorageVersionStatus
...
class StorageVersion(BaseModel):
    ...
    metadata: ObjectMeta = Field(
        default=None, description=r""" The name is <group>.<resource>. """
    )
    spec: StorageVersionSpec = Field(
        default=None,
        description=r""" Spec is an empty spec. It is here to comply with Kubernetes API style. """,
    )
    status: StorageVersionStatus = Field(
        default=None,
        description=r""" API server instances report the version they can decode and the version they encode objects to when persisting objects in the backend. """,
    )
