...
from k8s_models.metadata.certificates import ClusterTrustBundleSpec
...
class ClusterTrustBundle(BaseModel):
    ...
    metadata: ObjectMeta = Field(
        default=None, description=r""" metadata contains the object metadata. """
    )
    spec: ClusterTrustBundleSpec = Field(
        default=None,
        description=r""" spec contains the signer (if any) and trust anchors. """,
    )
